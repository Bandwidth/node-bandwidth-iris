...
function Client(accountId, userName, password, options){
  ...
  if(!userName){
    userName = options.userName || process.env.BANDWIDTH_API_USERNAME;
  }
  if(!password){
    password = options.password || process.env.BANDWIDTH_API_PASSWORD;
  }
  if(!options.apiEndPoint){
    options.apiEndPoint = process.env.BANDWIDTH_API_ENDPOINT || "https://api.inetwork.com";
  }
  if(!options.apiVersion){
    options.apiVersion = process.env.BANDWIDTH_API_VERSION || "v1.0";
  }
